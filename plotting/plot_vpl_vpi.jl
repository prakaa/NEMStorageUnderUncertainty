...
function add_legend!(
    ...
    Legend(
        fig[y_loc, x_loc],
        [v_lg, value_colors],
        [["Value of perfect information", "Value of perfect lookahead"], value_labels],
        ["Values", value_title];
        framevisible=false,
    )
    return nothing
...
function plot_value_of_lookahead_and_information_across_formulations(
    ...
    for (state, value) in pairs(all_data)
        ...
        title = "$energy MWh BESS - VPL & VPI - $state Prices, 2021"
        Label(fig[0, :]; text=title, fontsize=22, font="Source Sans Pro")
        filename = "$(state)_$(energy)_allformulations_vpl_vpi.pdf"
...
function plot_value_of_lookahead_and_information_for_each_formulation(
    ...
    for (state, value) in pairs(all_data)
        ...
        for formulation in unique(state_data.formulation)
            ...
            title_stem = "$energy MWh BESS VPL & VPI - "
            if all(ismissing.(unique(df.param)))
                ...
                filename = "$(state)_$(energy)_$(formulation)_vpl_vpi.pdf"
                fig = _makie_plot_each_formulation(df, title, scale, 0.0)
                save(joinpath(save_path, filename), fig; pt_per_unit=1)
            else
                params = unique(df.param)
                for param in params
                    filename = "$(state)_$(energy)_$(formulation)_$(param)_vpl_vpi.pdf"
...
plot_path = joinpath("results", "plots", "vpl_vpi")
if !ispath(plot_path)
    mkpath(plot_path)
end
...
plot_value_of_lookahead_and_information_for_each_formulation(
    joinpath("results", "data"), plot_path
)
plot_value_of_lookahead_and_information_across_formulations(
    joinpath("results", "data"), plot_path
)
