...
"""
function simulate_forecast2021_ArbDiscounted_NoDeg(
    power::Float64, energy::Float64, func_str::String
)
    @assert func_str in ("exp", "hyp")
    if func_str == "exp"
        discount_function = NEMStorageUnderUncertainty.exponential_discounting
        discount_rate = 0.1994
    elseif func_str == "hyp"
        discount_function = NEMStorageUnderUncertainty.hyperbolic_discounting
        discount_rate = 0.4203
    end
    ...
    @info("BESS $power MW $energy MWh cost $func_str discounting")
    @info("Simulating perfect foresight (no discounting)")
    d_lifetime = storage.energy_capacity * 365.0 * 10.0
    # Middle of the range capital cost for 2022-23
    pf_formulation = NEMStorageUnderUncertainty.ArbitrageThroughputPenalty(
        d_lifetime, 600000.0
    )
    ...
    formulation = NEMStorageUnderUncertainty.ArbitrageDiscounted(
        d_lifetime, 600000.0, discount_function, discount_rate
    )
    ...
    NEMStorageUnderUncertainty.results_to_jld2(
        joinpath(
            @__DIR__,
            "results",
            ("NSW_$(energy)MWh_ArbDiscounted_param$(func_str)_NoDeg_2021.jld2"),
        ),
        "forecast",
        "$(power)MW",
        df,
...
@assert !isempty(ARGS) "Provide power and energy capacity and discount function as arguments (in that order)"
args = parse.(Float64, ARGS[1:2])
simulate_forecast2021_ArbDiscounted_NoDeg(args[1], args[2], ARGS[3])
